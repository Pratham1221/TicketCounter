...
public class TicketCounter {
    ...
    public void userEvents() {
        if (currentUser.getMyShows().size() == 0) {
            ...
            int[] arr = new int[currentUser.getMyShows().size()];
            for (int i = 0; i < arr.length; i++) {
                arr[i] = i + 1;
            }
            for (int i = 0; i < currentUser.getMyShows().size(); i++) {
                System.out.println(arr[i] + " " + currentUser.getMyShows().get(i).display());
            }
        ...
        if (user == 1) {
            ...
            System.out.println(selectedEvent.displayDetiled());
            userMenu();
        } else {
            userMenu();
        }
    ...
    public void organiserEvents() {
        if (currentOrganiser.getMyShows().size() == 0) {
            ...
            int[] arr = new int[currentOrganiser.getMyShows().size()];
            for (int i = 0; i < arr.length; i++) {
                arr[i] = i + 1;
            }
            for (int i = 0; i < currentOrganiser.getMyShows().size(); i++) {
                System.out.println(arr[i] + " " + currentOrganiser.getMyShows().get(i).display());
            }
            System.out.printf("(1) Choose a show%n" + "(2) Go Back%n");
        }
        int user = input.nextInt();
    ...
    public void modifyOrganiserEvent(Event e) {
        ...
        if (user == 1) {
            ...
            String name = input.nextLine();
            e.setEventName(name);
            notifyChange(e, "Event Name", name);
            ...
            String time = input.nextLine();
            e.setTime(time);
            notifyChange(e, "date/time", date + " " + time);
            System.out.println("Change made Successfully");
        } else if (user == 3) {
